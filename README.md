# ✨ Youngsun Joung — AI Engineer (LLM · RAG · CV)

고려대학교 수학 석사 기반의 논리적 사고력과 실전 프로젝트 중심의 경험을 바탕으로  
**LLM·RAG·AI 시스템 설계·자동화**에 강점을 가진 AI Engineer입니다.  
K-디지털 해커톤 수상 경력과 3회 팀장 경험을 통해, 구조적 문제 해결과 협업 중심의  
AI 개발 프로세스를 구축해왔습니다.

- 📫 Email: **joungyoungsun20@gmail.com**  
- 🔗 LinkedIn: https://www.linkedin.com/in/youngsun-joung-5b0584345  
- ✍️ Velog: https://velog.io/@ys2357/posts  
- 📝 Portfolio: https://sapphire-cart-f52.notion.site/Portfolio  

---

## 🔧 Tech Stack

### **AI / NLP**
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-000000?style=flat&logo=chainlink&logoColor=white)
![LangGraph](https://img.shields.io/badge/LangGraph-1A73E8?style=flat)
![FAISS](https://img.shields.io/badge/FAISS-0099CC?style=flat)
![Chroma](https://img.shields.io/badge/Chroma-00C897?style=flat)

### **Backend / Infra**
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat&logo=supabase&logoColor=white)
![Upstash](https://img.shields.io/badge/Upstash-06B6D4?style=flat)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![GCP](https://img.shields.io/badge/Google_Cloud-4285F4?style=flat&logo=googlecloud&logoColor=white)

### **Tools**
![W&B](https://img.shields.io/badge/Weights_&_Biases-FFBE00?style=flat&logo=weightsandbiases&logoColor=black)
![LangSmith](https://img.shields.io/badge/LangSmith-2A2B2D?style=flat)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![VSCode](https://img.shields.io/badge/VSCode-007ACC?style=flat&logo=visualstudiocode&logoColor=white)

---

## 🗂️ Projects
- [compare-ai (BE)](https://github.com/YS-2357/compare-ai-be): 여러 상용 LLM 응답을 병렬로 비교·요약하는 Compare-AI 서비스의 백엔드.  
- [compare-ai (FE)](https://github.com/YS-2357/compare-ai-fe): 실시간 LLM 응답 시각화·비교를 위한 Compare-AI 서비스의 프론트엔드.  
- [2025-GEO-Page](https://github.com/gyurili/2025-GEO-Project): LLM + AI 이미지 기반의 GEO/SEO 최적화 상세페이지 자동 생성기.  
- [2025-RAG-Project](https://github.com/gyurili/2025-LLM-Project): LangChain 기반 문서 RAG 시스템(프롬프트 튜닝, FAISS/Chroma, Streamlit UI).  
- [2025-pill-detection](https://github.com/codeit-Al-Project1/pill_detection_ai): Faster R-CNN·YOLOv8 기반 알약 객체 감지·분류 모델.  
- [Coding_Test](https://github.com/YS-2357/Coding_Test): 백준·프로그래머스·LeetCode 문제 풀이 및 알고리즘 노트 아카이브.  

---

## 🎓 Education
- **M.S. Mathematics**, Korea University — GPA **4.24 / 4.5**  
  - Thesis: *Representations of the Temperley–Lieb Algebra*  
- **B.S. Mathematics**, Inha University — GPA **3.90 / 4.5**

---

## 💼 Experience
- **AI Engineer Intern**, Intalk — LLM 기반 보험 보장분석 GPT 연구·개발  
- **Team Lead (3×)**, Codeit AI Sprint — Object Detection / RAG / Generative AI 프로젝트 총괄  

---

## 🏆 Awards
- **wp 7회 K-디지털 트레이닝 해커톤 3위(389팀 중)** — 고용노동부 장관상  
- **2013 전국 대학생 수학 경시대회 동상** — 대한수학회  

---

## 📈 GitHub Stats
![Youngsun's GitHub stats](https://github-readme-stats.vercel.app/api?username=YS-2357&show_icons=true&theme=default)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=YS-2357&layout=compact&theme=default)
![GitHub Streak](https://streak-stats.demolab.com/?user=YS-2357&theme=default)
